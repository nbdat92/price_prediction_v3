...
# ---------- THRESHOLDS ----------
def auto_thresholds(summary_path: str):
    # summary_path truyền vào đã chứa symbol + tf
    import os, pandas as pd, numpy as np
    if os.path.exists(summary_path):
        s = pd.read_csv(summary_path)
        if {'thr_long','thr_short','deadzone'}.issubset(s.columns):
            return float(s['thr_long'].iloc[-1]), float(s['thr_short'].iloc[-1]), float(s['deadzone'].iloc[-1])
