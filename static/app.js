...
(() => {
  ...
  if (el.n) el.n.addEventListener('change', () => { state.n = +el.n.value || 300; refresh(); });
  ...
  async function loadSettings() {
    ...
    if (el.cfg.symbol) el.cfg.symbol.value = s.symbol || 'XAUUSD';
    ...
    try {
      ...
      if (latest && el.latest) {
        ...
        el.latest.textContent =
          `Latest @ ${latest.time} | ${latest.symbol}/${latest.tf} | close=${(+latest.close).toFixed(2)} | `
          + `prob_up=${(+latest.prob_up).toFixed(4)} | `
          + `thrL=${(+latest.thrL).toFixed(2)} thrS=${(+latest.thrS).toFixed(2)} `
          + `dz=${(+latest.dz).toFixed(2)} | signal=${sigLabel}`;
      }
    } catch (e) {
      console.error(e);
